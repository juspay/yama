...
import { GuardianConfig, OperationOptions, ReviewOptions, EnhancementOptions } from '../../src/types';
...
describe('Guardian', () => {
  ...
  beforeEach(() => {
    ...
    (ContextGatherer as jest.Mock).mockImplementation(() => mockContextGatherer);
    (CodeReviewer as jest.Mock).mockImplementation(() => mockCodeReviewer);
    (DescriptionEnhancer as jest.Mock).mockImplementation(() => mockDescriptionEnhancer);
  ...
  describe('initialize', () => {
    it('should initialize Guardian successfully', async () => {
      ...
      expect(BitbucketProvider).toHaveBeenCalledWith(mockConfig.providers.git.credentials);
      expect(mockBitbucketProvider.initialize).toHaveBeenCalled();
      expect(ContextGatherer).toHaveBeenCalledWith(mockBitbucketProvider, mockConfig.providers.ai);
      expect(CodeReviewer).toHaveBeenCalledWith(mockBitbucketProvider, mockConfig.providers.ai, mockConfig.features.codeReview);
      expect(DescriptionEnhancer).toHaveBeenCalledWith(mockBitbucketProvider, mockConfig.providers.ai);
      expect(logger.badge).toHaveBeenCalled();
      expect(logger.phase).toHaveBeenCalledWith('🚀 Initializing Yama...');
      expect(logger.success).toHaveBeenCalledWith('✅ Yama initialized successfully');
    });
    ...
    it('should handle initialization errors', async () => {
      ...
      await expect(guardianInstance.initialize()).rejects.toThrow('Initialization failed: Initialization failed');
      expect(logger.error).toHaveBeenCalledWith('Failed to initialize Yama: Initialization failed');
    });
    ...
    it('should initialize with custom config path', async () => {
      ...
      expect(configManager.loadConfig).toHaveBeenCalledWith('/custom/config.yml');
    });
  });
  ...
  describe('processPR', () => {
    ...
    it('should process PR with multiple operations successfully', async () => {
      ...
      mockDescriptionEnhancer.enhanceWithContext.mockResolvedValue({
        ...
        statistics: { originalLength: 7, enhancedLength: 20, completedSections: 1, totalSections: 1 }
      });
      ...
      expect(logger.operation).toHaveBeenCalledWith('PR Processing', 'completed');
      expect(logger.phase).toHaveBeenCalledWith('📋 Gathering unified context...');
    });
    ...
    it('should handle "all" operation type', async () => {
      ...
      mockDescriptionEnhancer.enhanceWithContext.mockResolvedValue({
        ...
        sectionsEnhanced: [],
        preservedItems: { media: 0, files: 0, links: 0 },
        statistics: { originalLength: 8, enhancedLength: 8, completedSections: 1, totalSections: 1 }
      });
    ...
    it('should use cached context when available', async () => {
      ...
      await guardianInstance.processPR({ ...mockOptions, operations: ['review'] });
    ...
    it('should handle operation failures gracefully', async () => {
      mockContextGatherer.getCachedContext.mockResolvedValue(null);
      mockContextGatherer.gatherContext.mockResolvedValue(mockContext as any);
      mockCodeReviewer.reviewCodeWithContext.mockRejectedValue(new Error('Review failed'));

      const result = await guardianInstance.processPR({ ...mockOptions, operations: ['review'] });
    ...
    it('should handle uninitialized Guardian', async () => {
      ...
      await uninitializedGuardian.processPR({ ...mockOptions, operations: ['review'] });
  ...
  describe('processPRStream', () => {
    ...
    const mockContext = {
      pr: globalThis.testUtils.createMockPR({ id: 12345, title: 'Test PR' }),
      identifier: { workspace: 'test-workspace', repository: 'test-repo', pullRequestId: 12345 }
    };
    ...
    it('should stream PR processing updates', async () => {
      ...
      const updates: any[] = [];
      for await (const update of guardianInstance.processPRStream(mockOptions)) {
        updates.push(update);
      }
    ...
    it('should handle streaming operation failures', async () => {
      ...
      mockContextGatherer.getCachedContext.mockResolvedValue(null);
      mockContextGatherer.gatherContext.mockResolvedValue(fullMockContext as any);
      mockCodeReviewer.reviewCodeWithContext.mockRejectedValue(new Error('Review failed'));
      ...
      const errorUpdate = updates.find(u => u.status === 'error' && u.operation === 'review');
    ...
    it('should handle streaming processing failures', async () => {
      mockContextGatherer.getCachedContext.mockRejectedValue(new Error('Context failed'));
  ...
  describe('Individual Operations', () => {
    ...
    describe('reviewCode', () => {
      ...
      it('should execute code review successfully', async () => {
        ...
        mockContextGatherer.gatherContext.mockResolvedValue(mockContext as any);
        mockCodeReviewer.reviewCodeWithContext.mockResolvedValue(mockReviewResult);
        ...
        expect(logger.operation).toHaveBeenCalledWith('Code Review', 'completed');
      });

      it('should handle review failures', async () => {
        mockContextGatherer.gatherContext.mockRejectedValue(new Error('Context failed'));

        await expect(guardianInstance.reviewCode(mockReviewOptions)).rejects.toThrow('Context failed');
    ...
    describe('enhanceDescription', () => {
      ...
      it('should execute description enhancement successfully', async () => {
        ...
        mockContextGatherer.gatherContext.mockResolvedValue(mockContext as any);
        mockDescriptionEnhancer.enhanceWithContext.mockResolvedValue(mockEnhancementResult);

        const result = await guardianInstance.enhanceDescription(mockEnhancementOptions);

        expect(result).toEqual(mockEnhancementResult);
        expect(logger.operation).toHaveBeenCalledWith('Description Enhancement', 'started');
        expect(logger.operation).toHaveBeenCalledWith('Description Enhancement', 'completed');
      });

      it('should handle enhancement failures', async () => {
        mockContextGatherer.gatherContext.mockRejectedValue(new Error('Context failed'));

        await expect(guardianInstance.enhanceDescription(mockEnhancementOptions)).rejects.toThrow('Context failed');
        expect(logger.operation).toHaveBeenCalledWith('Description Enhancement', 'failed');
      });
    });
  });
  ...
  describe('Feature Disabled Handling', () => {
    ...
    it('should skip disabled code review', async () => {
      ...
      const result = await (guardianInstance as any).executeCodeReview(mockContext, {});
      
      expect(result).toEqual({ skipped: true, reason: 'disabled in config' });
      expect(logger.info).toHaveBeenCalledWith('Code review is disabled in configuration');
    });

    it('should skip disabled description enhancement', async () => {
      const mockContext = { identifier: { pullRequestId: 12345 } };
      
      const result = await (guardianInstance as any).executeDescriptionEnhancement(mockContext, {});
      
      expect(result).toEqual({ skipped: true, reason: 'disabled in config' });
      expect(logger.info).toHaveBeenCalledWith('Description enhancement is disabled in configuration');
    });
  });
  ...
  describe('Unknown Operations', () => {
    ...
    it('should handle security-scan operation (not implemented)', async () => {
      ...
      const result = await (guardianInstance as any).executeOperation('security-scan', mockContext, {});
    ...
    it('should handle analytics operation (not implemented)', async () => {
      ...
      const result = await (guardianInstance as any).executeOperation('analytics', mockContext, {});
    ...
    it('should handle unknown operation', async () => {
      ...
      const result = await (guardianInstance as any).executeOperation('unknown-op', mockContext, {});
  ...
  describe('healthCheck', () => {
    ...
    it('should return healthy status when all components are healthy', async () => {
      mockBitbucketProvider.healthCheck.mockResolvedValue({ healthy: true, details: { status: 'ok' } });
    ...
    it('should return unhealthy status when component fails', async () => {
      mockBitbucketProvider.healthCheck.mockResolvedValue({ healthy: false, details: { error: 'Connection failed' } });
    ...
    it('should handle health check errors', async () => {
      mockBitbucketProvider.healthCheck.mockRejectedValue(new Error('Health check failed'));
  ...
  describe('Edge Cases and Error Handling', () => {
    it('should handle missing pullRequestId in context identifier', async () => {
      ...
      const mockContext = {
        identifier: { workspace: 'test', repository: 'test', pullRequestId: 123 },
        pr: globalThis.testUtils.createMockPR()
      };
    ...
    it('should handle context gathering with diff requirements', async () => {
      ...
      const context = await (guardianInstance as any).gatherUnifiedContext(options);
