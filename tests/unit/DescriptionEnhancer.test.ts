...
import { AIProviderConfig, EnhancementOptions, DescriptionEnhancementConfig } from '../../src/types';
...
+  );
+};`,
        fileChanges: ['src/auth/Login.js', 'src/auth/authService.js', 'src/components/Header.js']
      }),
    ...
    descriptionEnhancer = new DescriptionEnhancer(mockBitbucketProvider, mockAIConfig);
  ...
  describe('Constructor', () => {
    it('should create DescriptionEnhancer with providers', () => {
      ...
      expect((descriptionEnhancer as any).bitbucketProvider).toBe(mockBitbucketProvider);
...
- Added rate limiting for login attempts`
      ...
      const result = await descriptionEnhancer.enhanceWithContext(mockContext, options);

      expect(result).toEqual({
        originalDescription: mockContext.pr.description,
        enhancedDescription: expect.stringContaining('Changelog (Modules Modified)'),
        sectionsAdded: ['Changelog (Modules Modified)', 'Test Cases (What to be tested)', 'CAC Config Or Service Config Changes'],
        sectionsEnhanced: [],
        preservedItems: {
          media: 1,
          files: 0,
...
- Added authentication features`
    ...
    it('should handle AI service failures gracefully', async () => {
      ...
      await expect(descriptionEnhancer.enhanceWithContext(mockContext, {
        workspace: 'test',
        repository: 'test',
        pullRequestId: 123
      })).rejects.toThrow('Description enhancement failed: AI service unavailable');
    });
    ...
    it('should handle PR update failures', async () => {
      ...
      await expect(descriptionEnhancer.enhanceWithContext(mockContext, {
        ...
        repository: 'test',
        pullRequestId: 123
      })).rejects.toThrow('Description enhancement failed: Description update failed: Update failed');
    });
  });
...
- Added authentication`
      ...
      expect(result.enhancedDescription).toContain('![screenshot](screenshot.png)');
    ...
    it('should handle unicode and special characters', async () => {
      const unicodeContext = {
        ...mockContext,
        pr: { ...mockContext.pr, description: '🚀 Feature: 测试 emoji and unicode 🌟\n\n[文档](doc.md)' }
      };

      const mockResponse = {
        content: unicodeContext.pr.description + '\n\n## 📋 Changelog\n- Added 测试 feature'
      };
      ...
      const result = await descriptionEnhancer.enhanceWithContext(unicodeContext, {
        workspace: 'test',
        repository: 'test',
        pullRequestId: 123
      });
