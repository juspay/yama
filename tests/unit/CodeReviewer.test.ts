...
+  console.log(data); // Debug log
   return data;
    ...
    codeReviewer = new CodeReviewer(mockBitbucketProvider, mockAIConfig, mockReviewConfig);
  ...
  describe('Constructor', () => {
    it('should create CodeReviewer with providers', () => {
      ...
      expect((codeReviewer as any).bitbucketProvider).toBe(mockBitbucketProvider);
  ...
  describe('reviewCodeWithContext', () => {
    it('should perform comprehensive code review successfully', async () => {
        ...
        content: JSON.stringify({
          violations: [
            ...
            },
            {
              ...
              message: 'Function lacks proper error handling for getData() call',
              file: 'src/test.js',
      ...
      const result = await codeReviewer.reviewCodeWithContext(mockContext, options);
      ...
      expect(mockNeurolink.generate).toHaveBeenCalledWith(
        expect.objectContaining({
          input: { text: expect.stringContaining('COMPLETE CODE CHANGES (NO TRUNCATION)') },
    ...
    it('should handle AI service failures gracefully', async () => {
      ...
      await expect(codeReviewer.reviewCodeWithContext(mockContext, {
        workspace: 'test',
        repository: 'test',
        pullRequestId: 123
  ...
  describe('Private Methods Testing via Public Interface', () => {
    it('should properly build analysis prompt (tested via public method)', async () => {
      ...
      expect(aiCallArgs.input.text).toContain('COMPLETE CODE CHANGES (NO TRUNCATION)');
    });
  ...
  describe('Code Snippet Validation and Fixing', () => {
    it('should validate code snippets exist in diff', async () => {
      ...
      expect(result.violations).toHaveLength(1);
      expect(result.violations[0].code_snippet).toBe('+  console.log(data); // Debug log');
    });
...
+  console.log('new');
   return data;
      ...
      expect(result.violations[0].code_snippet).toBe("const password = 'hardcoded123';");
    });
  });
          ...
          violations: [
            {
              ...
              suggestion: 'const str = ```nested code block```; // Contains triple backticks'
            }
          ]
        })
      };
...
+  }
        ...
        fileDiffs: new Map([
          ['src/test.js', `@@ -10,6 +10,8 @@ class Example {
   constructor() {
     this.data = null;
   }
+  
