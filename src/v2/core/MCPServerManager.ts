...
export class MCPServerManager {
  ...
  async setupMCPServers(
    ...
    console.log(`   ⏱️  Timeout per server: ${timeoutMs / 1000}s`);
    console.log(
  ...
  private async setupJiraMCPWithRetry(
    ...
    try {
      ...
      console.warn(
        `   ⚠️  Jira MCP server package not found. Skipping integration.`,
      );
      return {
        success: false,
        toolsAvailable: [],
    ...
    for (let attempt = 1; attempt <= this.retryConfig.maxAttempts; attempt++) {
      try {
        ...
        lastError = (error as Error).message;
        console.warn(`   ⚠️  Jira attempt ${attempt} failed: ${lastError}`);
    ...
    console.warn(
      `   ⚠️  Jira MCP setup failed after ${this.retryConfig.maxAttempts} attempts`,
    );
  ...
  private async verifyRequiredTools(
    ...
    try {
      ...
      if (missingTools.length > 0) {
        console.warn(
          `   ⚠️  Missing ${serverName} tools: ${missingTools.join(", ")}`,
        );
      }
      ...
      console.warn(
        `   ⚠️  Tool verification failed for ${serverName}: ${(error as Error).message}`,
      );
      return {
        success: false,
        foundTools: [],
